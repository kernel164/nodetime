      ...
      db.admin(function(err, adminDb) {
        ...
        try {
          adminDb.authenticate(server.auth.username, server.auth.password, function(err, result) {
            if(err) return self.done(db, err);
            if(!result) return self.done(db, err);

            try {
              adminDb.serverStatus(function(err, serverStatus) {
                if(err) return self.done(db, err);
                if(!serverStatus) return self.done(db, err);
...
MongoDBMonitor.prototype.extractCollMetrics = function(coll, stats) {
  ...
  self.metric(coll, stats, scope, 'Collection size', 'size' ,'_KB', false);
  self.metric(coll, stats, scope, 'Average object size', 'avgObjSize', '_KB', false);
  self.metric(coll, stats, scope, 'Storage size', 'storageSize', '_KB', false);
  self.metric(coll, stats, scope, 'Index size', 'totalIndexSize', '_KB', false);
