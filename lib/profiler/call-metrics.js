...
CallMetrics.prototype.init = function(scope, group) {
  ...
  this.rpm = mm.findOrCreateMetric(scope, 'Requests per minute', null, 'sum');
  this.trpm = mm.findOrCreateMetric(scope, 'Total requests per minute', null, 'inc');

  this.epm = mm.findOrCreateMetric(scope, 'Errors per minute', null, 'sum');
