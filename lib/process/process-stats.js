...
ProcessStats.prototype.collectMetrics = function() {
  ...
  try { 
    ...
    if(self.agent.features.hostMetrics) {
      ...
      self.agent.metric('OS/' + host, 'Load average', os.loadavg()[0], null, 'avg'); 
      self.agent.metric('OS/' + host, 'Free memory', os.freemem() / 1048576, 'MB', 'avg');
    }
  }
  catch(err) { 
  ...
  if(cpuTime !== undefined && self.lastCpuTime !== undefined) {
    self.agent.metric('Process', 'CPU time', (cpuTime - self.lastCpuTime) / 1000, 'ms', 'avg');
  }
